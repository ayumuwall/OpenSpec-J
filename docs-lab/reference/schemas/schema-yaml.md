...
これは`openspec/changes/add-auth/specs/`配下の Markdown ファイルに一致します。

`generates`は次の glob 形式に対応します。

- **ワイルドカードと文字クラス**：`specs/**/*.md`、`review-[ab].md`など、`*`、`?`、`[`を含む値。
- **ブレース展開**：`review-{api,ui}.md`のような選択肢や、`file-{1..3}.md`のような範囲。
- **extglob**：`@(proposal|design).md`、`+(proposal|design).md`、`!(proposal|design).md`など。

**リテラルのファイル名**：先頭の`!`だけでは glob になりません。その名前のファイルは`generates: '!review.md'`と指定します。`(proposal|design).md`のような通常の括弧や、`review-{api}.md`のように要素が1つのブレースも、そのままのファイル名です。
...
| `tracks`      | 変更フォルダ内にある Markdown タスクファイルへの任意の相対パスまたは glob。既定値は`null`です。                               |
...
`tracks: "**/tasks.md"`などの glob は、`backend/tasks.md`や`frontend/tasks.md`など、一致する全ファイルを読み取ります。タスクと進捗を合算します。status と list でも同じファイルを追跡するには、アーティファクトの`generates`も同じ値にしてください。

一致するファイルがない場合や、タスク本文を含むチェックボックスがない場合、Apply はブロックされたままです。OpenSpec は次の形式のチェックボックスを数えます。
...
`-`、`*`、`+`、最大9桁の番号に`.`または`)`を続けた形式の、すべての Markdown リスト記号を使えます。行頭の空白は使用できます。既定スキーマが生成する、より厳密な形式は[spec-driven ページの tasks.md セクション](spec-driven/index.md#tasksmd)で定義します。

追跡対象ファイルによって apply の状態が決まります。

- **`blocked`**：一致するファイルがない、または読めるファイルにタスク本文付きのチェックボックスがない。
- **`ready`**：未完了タスクがある、または一部ファイルを読めず他のファイルからタスクを取得できた。
- **`all_done`**：すべての追跡対象タスクがチェック済みで、一致する全ファイルを読み取れた。

一致したファイルを読めない場合も、読めるファイルのタスクと進捗は保持します。ただし`all_done`にはしません。[Apply の JSON 出力](../cli.md#openspec-instructions)で、読めない各ファイルと理由を示します。
...
- 相対パス
- アーティファクト ID、依存関係、循環
- `apply.requires`の ID がすべてスキーマ内のアーティファクトを指すこと
- テンプレートファイル

`apply.requires`に不明な ID があるスキーマは読み込めず、それを使うすべてのコマンドでエラーになります。

`apply.tracks`がどのアーティファクトの`generates`とも完全一致しない場合は警告しますが、検証は失敗しません。2つの文字列の一致で追跡対象アーティファクトを特定するためです。`task.md`の誤記のほか、`tracks: tasks/main.md`と`generates: tasks/*.md`のように glob がファイルを生成しても文字列が異なる場合も該当します。apply は指定ファイルを読み取りますが、`openspec list`と`openspec status`は代わりに`tasks.md`を数えます。
