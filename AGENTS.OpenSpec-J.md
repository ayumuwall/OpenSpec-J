...
実施ポイント:
- `scope.md` のチェックボックスを進捗の唯一の記録として更新する。**scope にチェックボックス付きで記載された対象ファイルは、例外なく必ず1ファイルずつ処理する。**
  - 1ファイルごとに、upstream 差分の確認、既存の OpenSpec-J 差分の確認、必要な競合解消・翻訳・書式追従、反映漏れがないかの目視確認までを完了させる。
  - 上記が完了した直後に、そのファイルの項目だけを `- [ ]` から `- [x]` へ更新してから次のファイルへ進む。
  - 未確認、作業途中、未翻訳箇所あり、競合未解消、または差分の目視確認が未実施のファイルには `- [x]` を付けない。
  - 複数ファイルを推測や一括操作でまとめて `- [x]` にしない。競合マーカーが消えたこと、ビルドが通ったこと、または日本語を含むことだけを、そのファイルの完了根拠にしない。
  - セッションをまたぐ場合や途中コミットを作る場合も、完了済みの `[x]` と未完了の `[ ]` を正確に維持する。
  - ファイルごとのチェック時点では、ビルドやテストの実行を必須としない。ビルド・テストなどの全体検証は、すべての対象ファイルの作業後に §2.5 で実施する。
  - **全チェックボックスが `[x]` になるまでは、ローカライズ作業全体を「完了」と報告せず、§2.5 または次工程へ進まない。**
...
- **上記コマンドは実行するだけでなく、原則としてすべて成功させる。**
  ...
  - 「テストを実行した」「失敗理由を把握した」「大部分が成功した」だけでは検証完了とみなさない。
- テストが失敗した場合は、失敗ごとに原因を切り分けて修正し、全体を再実行する。
  - 日本語化した表示文言に対して英語を期待しているテストは、§0.3 の「日本語版テスト検証に必要な場合」に該当するため、`test/` が原則翻訳対象外であっても期待値を日本語版へ更新する。
  - upstream の仕様・ロジック不具合、OpenSpec-J の実装不具合、翻訳後の期待値不一致、実行環境固有の失敗を混同しない。
  - sandbox や権限制限（例: localhost の listen 不許可）が原因と疑われる場合は、許可された環境で該当テストを再実行して、コード起因でないことを確認する。単に「環境制約」と推測して成功扱いにしない。
  - 外部サービス、認証、対応OSなどが必要で、現在の環境ではどうしても成功確認できないテストが残る場合は、独断で完了扱いにせず作業を停止し、失敗件数・対象テスト・再現コマンド・必要な利用者対応を報告する。
- 修正後は、失敗したテストだけでなく `pnpm test` を再実行し、他のテストへの回帰がないことを確認する。
- 最終報告と `SESSION_MEMO.md` には、成功した検証コマンドと全テスト件数を記録する。失敗を含む途中結果を最終的な検証結果として記録しない。
- 主要コマンドのヘルプ文言を目視確認
...
- **scope.md の `[x]` 項目について、ファイルごとに `git diff upstream-vA.B.C upstream-vX.Y.Z -- <file>` を走らせ、upstream 側の変更が ja HEAD にも反映されているかを 1 件ずつ目視確認する**。書式のみの変更（padding / 見出し改名など）も必ず取り込む。`git diff v<前 ja 版> HEAD -- <file>` が空なのに upstream 差分が非空なら、ほぼ確実に作業漏れ。
  - この最終確認でも一括判定は行わない。各ファイルの確認結果と `scope.md` のチェック状態が一致していることを照合する。
  - `[x]` のファイルに作業漏れが見つかった場合は、直ちにその項目を `[ ]` へ戻し、修正と差分の目視確認が終わってから再度 `[x]` にする。
