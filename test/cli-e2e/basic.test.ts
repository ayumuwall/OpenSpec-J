...
describe('openspec CLI e2e basics', () => {
  it('preserves initialized directories through a Git clone without listing anchors as work', async () => {
    ...
    expect(specs.stdout).toContain('仕様が見つかりません。');
  });
  ...
  describe('legacy change list compatibility', () => {
    it.each([
      { args: [], output: 'c1\n' },
      { args: ['--long'], output: 'c1: Test Change [差分 1]\n' },
      ...
      expect(result.stderr).toContain('警告: "openspec change list" は非推奨です。"openspec list" を使ってください。');
    });
  ...
  describe('init command non-interactive options', () => {
    ...
    it('initializes with --tools all option', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('OpenSpec 構成を作成しました');
    ...
    it('initializes with --tools zed option', async () => {
      ...
      expect(result.stdout).not.toContain('IDE を再起動');
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('無効なツール: invalid-tool');
    expect(result.stderr).toContain('利用可能な値:');
    });
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('予約値 "all" / "none" を特定のツールIDと併用できません');
  });
  });
