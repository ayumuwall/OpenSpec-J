...
`
  ...
  describe('default resolution without --store', () => {
    ...
    it('says so when the first change creates the root in an unset-up directory', async () => {
      ...
      expect(firstOutput).toContain('OpenSpec ルートが見つからなかった');
      ...
      expect(firstOutput).toContain('openspec/ に作成しました。');
      ...
      expect(second.stdout + second.stderr).not.toContain('OpenSpec ルートが見つからなかった');
    });
    ...
    it('names the directory it actually adopted when run from a subdirectory', async () => {
      ...
      expect(created.exitCode).toBe(0);
      expect(created.stdout + created.stderr).toContain('openspec/ に作成しました。');
    ...
    it('keeps the notice out of JSON output', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).not.toContain('OpenSpec ルートが見つからなかった');
      ...
      for (const args of [
        ...
        expect(json.status[0]).toEqual(
          expect.objectContaining({
            ...
            message: '現在のディレクトリから OpenSpec ルートが見つかりません。',
          })
        );
      }
    });
