...
`);
  ...
  for (const strict of [false, true]) {
    for (const args of entryPoints) {
      ...
      it(`shows non-blocking archive advice for ${invocation.join(' ')}`, async () => {
        ...
        expect(result.stderr).toContain('ℹ [INFO] widgets/spec.md: アーカイブでこの仕様差分は拒否されます:');
        expect(result.stderr).toContain('Future state');
        expect(result.stderr).not.toContain('次のステップ:');
        expect(result.stdout).toMatch(/は有効です|失敗 0/);
      });
      ...
      it(`keeps archive advice structured and non-blocking for ${invocation.join(' ')} --json`, async () => {
        ...
        expect(report.issues).toContainEqual(expect.objectContaining({
          ...
          message: expect.stringContaining('アーカイブでこの仕様差分は拒否されます:'),
        }));
        expect(result.stderr).not.toContain('アーカイブでこの仕様差分は拒否されます:');
      ...
      it.skipIf(process.platform === 'win32')(
        ...
        async () => {
          ...
          if (json) {
            ...
            expect(result.stdout).toMatch(/は有効です|失敗 0/);
            expect(result.stderr).toContain('ℹ [INFO] specs: アーカイブ時のマージ競合を検査できませんでした:');
            expect(result.stderr).not.toContain('次のステップ:');
          }
          expect(result.exitCode).toBe(0);
        }
  ...
  it('preserves INFO severity in the deprecated command when another delta is invalid', async () => {
    ...
    expect(result.stderr).toContain('次のステップ:');
  });
  ...
  it('prints Next steps footer and guidance on invalid change', async () => {
    ...
    try {
      ...
      expect(stderr).toContain('に問題があります');
