...
describe('workspace command', () => {
  ...
  it('does not route openspec update through workspace update from a workspace root', async () => {
    ...
    expect(`${update.stdout}\n${update.stderr}`).toContain('`openspec workspace update` を実行してください');
    expect(update.stdout).not.toContain('Workspace update が完了しました');
  ...
  it('updates repo-local project targets nested under a workspace without touching workspace state', async () => {
    ...
    expect(update.exitCode).toBe(0);
    expect(update.stdout).toContain('設定済みのツールが見つかりません');
    expect(`${update.stdout}\n${update.stderr}`).not.toContain('`openspec workspace update` を実行してください');
    expect(update.stdout).not.toContain('Workspace update が完了しました');
    expect(fs.readFileSync(getWorkspaceViewStatePath(workspaceRoot), 'utf-8')).toBe(workspaceStateBefore);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'openspec-propose', 'SKILL.md'))).toBe(false);
  });
...
`;
    ...
    expect(update.exitCode).toBe(0);
    expect(update.stdout).not.toContain('Workspace update が完了しました');
    expect(update.stderr).not.toContain('workspace 状態が不正です');
    expect(update.stdout).toContain('設定済みのツールが見つかりません');
  ...
  it('does not update a workspace passed to openspec update even when another workspace is known', async () => {
    ...
    expect(`${update.stdout}\n${update.stderr}`).toContain('`openspec workspace update` を実行してください');
    expect(update.stdout).not.toContain('Workspace update が完了しました');
    expect(update.stdout).not.toContain('複数の OpenSpec workspace が登録されています');
...
`
  ...
  it('fails setup cleanly for missing automation inputs and JSON without no-interactive', async () => {
    ...
    expect(noWorkspaces.stdout).toContain("OpenSpec workspace は見つかりませんでした。まず 'openspec workspace setup' を実行してください。");
  ...
  it('links, rejects duplicate link names, relinks, and reports unknown relinks', async () => {
    ...
    expect(parseJson(duplicateResult).status[0]).toEqual(
      expect.objectContaining({
        code: 'duplicate_link_name',
        message: expect.stringContaining('既に同じ名前を使っている'),
      })
    );
  ...
  it('reports malformed workspace state in list and doctor without rewriting files', async () => {
    ...
    expect(humanList.stdout).toContain('workspace の状態を読み取れませんでした');
...
`;
  ...
  it('fails non-interactive ambiguous workspace selection in human output mode', async () => {
    ...
    expect(doctor.exitCode).toBe(1);
    expect(doctor.stderr).toContain('複数の OpenSpec workspace が登録されています');
    expect(doctor.stderr).toContain('--workspace <name> を渡してください');
  ...
  it('reports workspace open selection errors', async () => {
    ...
    expect(noKnown.exitCode).toBe(1);
    expect(noKnown.stderr).toContain("既知の OpenSpec workspace はありません。先に 'openspec workspace setup' を実行してください。");
    ...
    expect(conflict.stderr).toContain("位置引数 'platform'");
    ...
    expect(ambiguous.stderr).toContain('既知の workspace: checkout-web, platform');
  ...
  it('reports unsupported workspace open options before workspace selection', async () => {
    ...
    expect(unsupported.stderr).toContain('将来の context/query 機能');
    ...
    expect(changeUnsupported.stderr).toContain('workspace ルートを開く操作だけ');
    ...
    expect(openerConflict.stderr).toContain('--agent <tool> または --editor のどちらか一方');
  });
...
`
    ...
    expect(unavailable.stderr).toContain("PATH 上に 'code' が見つかりませんでした");
  ...
  it('prints readable human output for setup, list, and doctor', async () => {
    ...
    expect(setup.exitCode).toBe(0);
    expect(setup.stdout).toContain('Workspace setup が完了しました');
    expect(setup.stdout).toContain('OpenSpec workspaces (1)');
    expect(setup.stdout).toContain('場所:');
    expect(setup.stdout).not.toContain('Root:');
    expect(setup.stdout).toContain('リンク済みリポジトリまたはフォルダ (1):');
    expect(setup.stdout).toContain(`api -> ${expectedApi}`);
    expect(setup.stdout).toContain('Workspace 確認:');
    expect(setup.stdout).toContain('workspace の問題は見つかりませんでした。');
    expect(setup.stdout).toContain('次に使えるコマンド:');
    ...
    expect(list.stdout).toContain('場所:');
    expect(list.stdout).not.toContain('Root:');
    expect(list.stdout).toContain('リンク済みリポジトリまたはフォルダ (1):');
    ...
    expect(doctor.stdout).toContain('場所:');
    expect(doctor.stdout).not.toContain('Root:');
    expect(doctor.stdout).toContain('リンク済みリポジトリまたはフォルダ:');
    expect(doctor.stdout).toContain('workspace の問題は見つかりませんでした。');
  });
  ...
  it('does not expose workspace create as a public command', async () => {
    ...
    expect(updateHelp.stdout).toContain('ガイダンスとエージェントスキル');
    expect(updateHelp.stdout).toContain('--workspace');
    expect(updateHelp.stdout).toContain('--tools');
    expect(updateHelp.stdout).toMatch(/Global\s+profile が workflows/u);
  });
  ...
  it('registers workspace subcommands for shell completions', () => {
    ...
    expect(update?.description).toContain('ガイドとエージェントスキル');
