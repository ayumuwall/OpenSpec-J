...
describe('config edit', () => {
  ...
  it('reports an editor that exits non-zero instead of throwing', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(errors()).toContain('がコード 3 で終了しました');
  }, T);
  ...
  it('still validates the file once the editor closes', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(errors()).toContain('JSON が無効');
  }, T);
  ...
  it('reports when no editor is configured', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(errors()).toContain('エディタが設定されていません');
  }, T);
  ...
  it('reports an 閉じられていない引用符 without starting anything', async () => {
    process.env.EDITOR = `"${process.execPath} --wait`;

    await runConfigCommand(['edit']);

    expect(process.exitCode).toBe(1);
    expect(errors()).toContain('閉じられていない引用符');
  ...
  it('shows the install hint for a missing editor', async () => {
    ...
    expect(errors()).toContain('EDITOR または VISUAL にインストール済みのエディタのコマンドを設定してください');
  }, T);
  ...
  it.skipIf(process.platform === 'win32')('omits the install hint when the editor exists but cannot run', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(errors()).toContain('を起動できませんでした');
    expect(errors()).not.toContain('EDITOR または VISUAL にインストール済みのエディタのコマンドを設定してください');
  }, T);
});
...
describe('splitEditorCommand', () => {
  ...
  it('returns null for an 閉じられていない引用符 and nothing for a blank value', () => {
