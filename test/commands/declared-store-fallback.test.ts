...
describe('declared store fallback (3.2)', () => {
  ...
  it('runs the externalized-planning journey without --store anywhere', async () => {
    ...
    expect(statusHuman.stdout).toContain(
      '次: openspec instructions proposal --change "billing-rework" --store team-context --json'
    );
  ...
  it('refuses init in a pointer repo and creates nothing, then converts cleanly', async () => {
    ...
    expect(refusedWithLanguage.stderr).toContain("ストア 'team-context' に外部化されています");
    expect(refusedWithLanguage.stderr).toContain('store: 行を削除');
    expect(snapshot(pointerRepo)).toEqual(before);
