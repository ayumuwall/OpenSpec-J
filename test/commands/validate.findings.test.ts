...
describe('validate findings reports', () => {
  ...
  it('orders human streams independently and emits every issue without clean rows', async () => {
    ...
    expect(human.stdout[0]).toMatch(/^対象範囲:/);
    expect(human.stdout[1]).toMatch(/^合計:/);
    expect(human.stdout[2]).toMatch(/^詳細: openspec validate d-error --type change/);
  ...
  it.each(scopes)('keeps empty %s scopes explicit and successful (%j)', async (scope, options) => {
    ...
    expect(human.stdout).toEqual([expect.stringMatching(/^対象範囲:/), '検出事項のある項目はありません。', '合計: 0件成功、0件失敗（全0件）']);
  ...
  it('distinguishes a clean non-empty scope from an empty scope', async () => {
    ...
    expect(human.stdout).toEqual([expect.stringMatching(/^対象範囲:/), '検出事項のある項目はありません。', '合計: 1件成功、0件失敗（全1件）']);
    expect(human.stderr).toEqual([]);
    expect(human.exitCode).toBe(0);
  });
  ...
  describe('a namespace folder holding nested changes (#1846)', () => {
      ...
      expect(item.issues).toEqual([
        {
          level: 'ERROR',
          path: 'file',
          message: expect.stringContaining('"mobile" は変更ではなく'),
        },
      ]);
    ...
    it('replaces the delta-authoring next steps with the flat-layout fix', async () => {
      ...
      expect(output).toContain('入れ子の各変更を openspec/changes/ の直下へ移動');
