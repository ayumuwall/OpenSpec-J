...
describe('formatOptionalWorkflowsNote', () => {
  it('names every workflow the core profile leaves out', () => {
    ...
    expect(note?.[0]).toBe(
      '補足: ほかに 6 件のワークフローを利用できます（new, continue, ff, bulk-archive, verify, onboard）。'
    );
    expect(note?.[1]).toBe(
  ...
  it('lists the missing workflows in declaration order, not the order given', () => {
    ...
    expect(note?.[0]).toBe('補足: ほかに 2 件のワークフローを利用できます（new, verify）。');
  });
  ...
  it('reads as a singular sentence when exactly one workflow is missing', () => {
    ...
    expect(note?.[0]).toBe('補足: ほかに 1 件のワークフローを利用できます（onboard）。');
    expect(note?.[1]).toBe(
      '`openspec config profile` で追加できます。'
    );
  });
