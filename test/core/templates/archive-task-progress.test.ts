...
describe('archive task discovery uses schema-resolved CLI progress', () => {
  ...
  for (const [surface, content] of surfaces) {
    it.each([
      ...
      expect(step).toContain('同じ選択済みルートのフラグ');
      expect(step).toMatch(/`name` が(?:選択した変更と)?完全一致する/);
      expect(step).toContain('totalTasks - completedTasks');
      expect(step).toContain('非負の整数');
      expect(step).toContain('completedTasks <= totalTasks');
      expect(step).toMatch(/それ以外.*未完了/s);
      ...
      const storeFlag = content.match(/`(--store) <id>` を指定します/);
      expect(storeFlag).not.toBeNull();
      expect(content).toContain('以下にフラグなしで示すコマンド例は省略形なので、実行前にフラグを追加');
