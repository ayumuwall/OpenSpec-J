...
describe('project root guard', () => {
  ...
  it('leaves the rootless feedback skill alone', () => {
    expect(getFeedbackSkillTemplate().instructions).not.toContain('**プロジェクトの確認:**');
  });
  ...
  it('names the machine-readable signal rather than a guess', () => {
    ...
    expect(PROJECT_ROOT_GUARD).toContain('ストアを選択している場合は、そのストアがルートになるため `--store <id>` を指定');
    ...
    expect(PROJECT_ROOT_GUARD).toContain('コマンドは非ゼロで終了しますが、CLI の故障ではなく確認結果');
  });
  ...
  it('does not mistake an unregistered declared store for an uninitialized project', () => {
    expect(PROJECT_ROOT_GUARD).toContain('「での宣言:」または「のストア宣言が不正です:」と続く');
    expect(PROJECT_ROOT_GUARD).toContain('未初期化として扱わず、以下の分岐には進まない');
    expect(PROJECT_ROOT_GUARD).toContain("エラーの `message` と `fix` をユーザーに示します");
    expect(PROJECT_ROOT_GUARD.indexOf('「での宣言:」または「のストア宣言が不正です:」と続く')).toBeLessThan(
      PROJECT_ROOT_GUARD.indexOf('**自動選択した場合**')
    );
  });
  ...
  it('gets out of the way when it selected itself', () => {
    const autoSelected = branch('**自動選択した場合**');

    expect(autoSelected).toContain('ユーザーが OpenSpec やこのスキルを指定せず');
    expect(autoSelected).toContain('通常どおり依頼に対応');
    // The reported bug is being asked to choose a setup path for a project the
    // user never said was an OpenSpec project.
    expect(autoSelected).toContain('セットアップを求めたり');
  ...
  it('asks when the user named OpenSpec, this skill, or its command', () => {
    const explicit = branch('**OpenSpec を明示的に指定された場合**');

    expect(explicit).toContain('OpenSpec やこのスキルを指定した、またはスラッシュコマンドを実行した');
    expect(explicit).toContain('書き込み前に停止し、このプロジェクトをセットアップする');
    expect(explicit).toContain('`openspec init`');
    expect(explicit).toContain('`--store <id>`');
    expect(explicit).toContain('OpenSpec を使わずに進める');
    expect(explicit).toContain('回答を待ちます');
  });
  ...
  it('carries the explicit branch into every deployed opsx command', () => {
    for (const [label, body] of renderedBodies()) {
      ...
      expect(body, label).toContain('**OpenSpec を明示的に指定された場合**');
      expect(body, label).toContain('書き込み前に停止し、このプロジェクトをセットアップする');
    }
  });

  it('never lets any branch create the root as a side effect', () => {
    expect(PROJECT_ROOT_GUARD).toContain('どちらの場合も、副作用としてルートを作成してはいけません');
    expect(PROJECT_ROOT_GUARD).toContain('ユーザーが依頼するまでは `openspec init` を実行せず');
    expect(PROJECT_ROOT_GUARD).toContain('`openspec/` 配下のファイルを手作業で作成せず');
    expect(PROJECT_ROOT_GUARD).toContain('コマンドによる自動作成も避けてください');
  });
