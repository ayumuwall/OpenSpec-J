...
describe('spec inventory vocabulary (#1689)', () => {
  ...
  it('distinguishes the change list from the spec inventory in explore', () => {
    ...
    for (const [label, body] of exploreBodies) {
      ...
      expect(body, label).toContain('`openspec list` だけでは表示されません');
    }
  });
  ...
  it('reads a listed capability with the store-aware command', () => {
    ...
    for (const [label, body] of sites) {
      ...
      expect(readContext, `${label} does not apply the store rule to the read`).toMatch(
        /(?:`--store` の指定条件は同じ|登録済みの独立ストアを使う場合だけ、両コマンドに `--store "<id>"` を付ける)/
      );
    }
  });
  ...
  it('reads full relevant specs before deciding coverage or changes', () => {
    ...
    for (const [label, body] of sites) {
      ...
      expect(normalized, label).toContain('シナリオを除いた読み取りは概要の確認に限');
      expect(normalized, label).toContain('既存の対応範囲や変更内容を判断する前に');
      expect(normalized, label).toContain('`openspec show "<spec-id>" --type spec`');
      expect(normalized, label).toContain('シナリオも含めて全文読み取');
      expect(normalized, label).toContain('`--store` の指定条件は同じ');
    }
  });
});
