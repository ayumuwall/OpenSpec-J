...
`
    ...
    it('does not report a change with unfinished sub-tasks as complete (#1485)', async () => {
      ...
      expect(logOutput.some(line => line.includes('1/2 タスク'))).toBe(true);
      expect(logOutput.some(line => line.includes('✓ 完了'))).toBe(false);
    });
    ...
    it('does not report a change whose remaining work uses an unrecognised marker as complete (#1761)', async () => {
      ...
      expect(logOutput.some(line => line.includes('1/3 タスク'))).toBe(true);
    ...
    describe('a namespace folder holding nested changes (#1846)', () => {
      ...
      it('is listed as "not a change" instead of a task-less change', async () => {
        ...
        expect(
          logOutput.some(line => line.includes('mobile') && line.includes('変更ではありません'))
        ).toBe(true);
        expect(
          logOutput.some(line => line.includes('mobile') && line.includes('タスクなし'))
        ).toBe(false);
      });
      ...
      it('explains the nesting and how to fix it', async () => {
        ...
        const warning = logOutput.find(line => line.startsWith('警告:'));
      ...
      it('still lists the real changes around it', async () => {
        ...
        expect(
          logOutput.some(line => line.includes('add-auth') && line.includes('完了'))
        ).toBe(true);
      });
