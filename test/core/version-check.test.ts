...
describe('getAvailableCliUpdate', () => {
  ...
  it('asks the dist-tag endpoint, and never with an Accept type it answers 406 for', async () => {
    ...
    expect(requests[0].url).toBe('/@ayumuwall/openspec/latest');
  ...
  it('follows a redirect, as mirrors and corporate front-ends send', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/elsewhere/@ayumuwall/openspec/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
    ...
    await expect(getAvailableCliUpdate()).resolves.toBe(bumpMajor(OPENSPEC_VERSION));
    expect(requests[1].url).toBe('/elsewhere/@ayumuwall/openspec/latest');
  });
  ...
  it('tears down a redirected connection when the overall budget expires', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/mirror/@ayumuwall/openspec/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('asks the registry npm exported, and only that', () => {
    ...
    try {
      ...
      expect(registryUrl()).toBe('https://registry.npmjs.org/@ayumuwall/openspec/latest');

      process.env.npm_config_registry = 'https://env.example.com';
      expect(registryUrl()).toBe('https://env.example.com/@ayumuwall/openspec/latest');
    } finally {
  ...
  it('falls back to the public registry when the override is not an http(s) URL', () => {
    ...
    expect(registryUrl()).toBe('https://npm.internal.example.com/@ayumuwall/openspec/latest');
  });
});
...
describe('displayCliUpdateNote', () => {
  ...
  it('names the global install command and the copy that answered', () => {
    ...
    expect(output).toContain('npm install -g @ayumuwall/openspec@latest');
    expect(output).toContain('その後、"openspec update" を再実行');
    expect(output).toContain(`実行元: ${getInstallDir()}`);
  });
  ...
  it('picks the upgrade command that matches how the CLI was installed', () => {
    ...
    expect(globalLines).toContain('npm install -g @ayumuwall/openspec@latest');
    ...
    expect(local).toContain('このプロジェクトの @ayumuwall/openspec 依存関係を更新してください。');
    ...
    expect(npx).toContain('npx @ayumuwall/openspec@latest update');
  ...
  it('does not tell npx users to run an update they were just handed', () => {
    ...
    expect(npx).toEqual(['  npx @ayumuwall/openspec@latest update']);
    ...
    expect(buildUpgradeCommandLines(path.join(GLOBAL_ROOT, 'lib', 'node_modules', 'pkg'), PROJECT_ROOT))
      .toContain('  その後、"openspec update" を再実行して新しいワークフローを取り込んでください。');
  });
