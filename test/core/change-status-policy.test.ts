  ...
  describe('an artifact is ready', () => {
    it('names that artifact instructions command', () => {
      expect(resolveNextStep(input())).toEqual({
        ...
        sentence:
          'そのアーティファクトを書き始める前に、openspec instructions specs --change "add-dark-mode" --json を実行してください。',
      });
    });
  ...
  describe('planning is complete', () => {
    it('names the apply instructions command', () => {
      ...
      expect(step).toEqual({
        ...
        sentence:
          'すべての計画アーティファクトが完了しました。実装の進捗を確認するには、openspec instructions apply --change "add-dark-mode" --json を実行してください。',
      });
    });
  });
