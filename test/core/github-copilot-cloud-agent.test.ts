...
const MANAGED_MARKER = 'OpenSpec が GitHub Copilot コーディングエージェント用に生成しました。';
...
describe('GitHub Copilot Cloud Agent', () => {
  ...
  function removeManagedMarker(content: string): string {
    const withoutMarker = content
      .replace(/^# OpenSpec が GitHub Copilot コーディングエージェント用に生成しました。\n\n/, '')
      .replace(/\n<!-- OpenSpec が GitHub Copilot コーディングエージェント用に生成しました。 -->\n/, '');
    ...
    it('generates a structurally valid Copilot setup workflow', () => {
      ...
      expect(workflow.jobs['copilot-setup-steps'].steps).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ run: 'npm install -g @ayumuwall/openspec' }),
  ...
  describe('generateCopilotAgentFile', () => {
    it('generates valid agent frontmatter and non-interactive guidance', () => {
      ...
      expect(content).toContain('OpenSpec が GitHub Copilot コーディングエージェント用に生成しました。');
      expect(content).toContain('# OpenSpec エージェント');
      ...
      expect(content).toContain('すべてのタスク完了を確認した後だけ `--yes` を使用');
      expect(content).toContain('`openspec --version` を実行');
  ...
  describe('writeCopilotCloudFiles', () => {
    ...
    it('creates robust agent guidance alongside a customized setup workflow', async () => {
      ...
      expect(agentContent).toContain('`openspec --version` を実行');
      expect(agentContent).toContain('`npm install -g @ayumuwall/openspec` でインストール');
    ...
    it('rejects a directory at a managed file path before creating other files', async () => {
      ...
      await expect(writeCopilotCloudFiles(tempDir)).rejects.toThrow(
        '管理対象の Copilot パスが通常ファイルではありません'
      );
