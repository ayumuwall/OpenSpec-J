...
describe('explore documentation', () => {
  ...
  const USER_TRIGGER = /\b(you ask|accept its offer|say yes)\b|利用者が依頼した場合|提案に同意した場合/i;
  const NAMED_SCOPE = /\bartifacts you name(d)?\b|指定された計画アーティファクト/i;
  ...
  for (const page of ['docs/explore.md', 'docs/commands.md']) {
    it(`${page} still documents the user-requested capture contract (#1833)`, () => {
      ...
      const captureLines = lines.filter((line) => /\bcaptur|探索内容を記録/i.test(line));
