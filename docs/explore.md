...
**`/opsx:explore` は、計画を作る前に使う思考パートナーです。問題はあるが、まだ計画がないときに使います。** コードを書く前に、コードベースを調べ、選択肢を比較し、本当に必要な変更を明確にします。方向性が見えたら `/opsx:propose` に引き継ぎます。
...
- 利用者が依頼した場合や、記録の提案に同意した場合は、探索内容を記録する。`openspec new change` で変更のひな形を作成して指定された計画アーティファクトを書き込むか、既存の変更のアーティファクトを更新する。
- 準備ができたら `/opsx:propose` へつなぐ。

**Explore がしないこと:**

- コードを作成・変更する。探索内容を記録する場合を含め、どの経路でもコードは書きません。
- スキーマやテンプレートを設計・編集する。これらを形にする作業は、思考ではなく変更に当たります。
- 独断で変更を開始したり、アーティファクトを書いたりする。依頼や提案への同意がある場合に限り、合意した内容と、変更の開始に必要なセットアップファイル（後述）だけを書き込みます。
- 記録を強く勧める。考えがまとまったところで提案しますが、記録するかどうかは利用者が決めます。

そこが重要です。探索は安く、方針を決めるまでは後戻りも簡単です。3 つの行き止まりを試し、それぞれから学んでから、生き残った選択肢だけを提案にできます。
...
会話を続けたまま、Explore に変更の記録を依頼することもできます。「これについて変更を開始して」でフォルダーのひな形を作成し、「提案も書いて」で指定されたアーティファクトだけを書き込みます。ひな形作成時には変更固有のメタデータも作成し、プロジェクトの最上位で不足している構成（`openspec/specs/`、`openspec/changes/archive/`、`config.yaml`）を補います。

引き継いだ場合と同じように変更を記録できますが、作成範囲が異なります。Propose は実装に進むためにスキーマが要求する一式を書きます。Explore での記録は、指定されたアーティファクトだけを書きます。
...
**得られるもの:** 探索は、方針を決める前の最も安いタイミングで、方向違いを見つけます。慣れていないコードでは特に効果があります。AI に読ませて要約させることで、午後いっぱいの調査を避けられることがあります。
