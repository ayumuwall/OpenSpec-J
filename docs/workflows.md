...
Explore はコードを書きません。それ以外の書き込みも、依頼された場合や、提案に同意を得た場合だけ行います。曖昧な不安を具体的な変更へ近づける、リスクのない会話です。そのため、後続の提案が鋭くなります。何を作るべきかすでに明確なら、Explore を省略して `/opsx:propose` に直接進んで構いません。詳しくは [まずは探索する](explore.md) を参照してください。
