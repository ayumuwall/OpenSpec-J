...
**マシン上の全リポジトリに共通するデフォルト。** 複数のコードリポジトリから
同じstoreへ計画を集約する場合、各リポジトリに `store:` を追加する代わりに、
グローバル設定を一度行えます。
...
これで、計画ルート外から、`--store` もプロジェクトポインターも指定せずに
実行したコマンドは `team-plans` を使います。優先順位は最も低いため、
`--store`、ローカルルート、プロジェクトの `store:` ポインターが優先されます。
ルートバナーとJSONの `root` ブロックにはstore IDとともに
`source: "global_default"` が表示され、マシン共通のデフォルトと
リポジトリ固有のポインターを区別できます。
解除するには `openspec config unset defaultStore` を実行します。
未登録のIDの場合はエラーとなり、登録または古いデフォルトの解除を案内します。

## 例: 1 つの機能、2 つのコンポーネントリポジトリ

`add-checkout-promo` が `checkout-api` と `checkout-web` の両方を変更する
とします。チームは共有のプロダクト契約を 1 つ持ちたい一方、各コード
リポジトリにはそれぞれ固有の実装タスク、ブランチ、レビューが必要です。

2 層に分けます。

1. 共有する振る舞いは `team-plans` に置きます。
2. 実装計画は各コンポーネントリポジトリに置き、ストアを読み取り専用の
   上流コンテキストとして参照します。
...
提案と仕様では、コンポーネント間の境界にある振る舞いを記述します。たとえば、
サービスが返すプロモーション項目や、対象外のチェックアウトをフロントエンドが
どう扱うかです。この変更は、他のブランチやプルリクエストと同様にストア
リポジトリでレビューします。

### 計画時に見えるコンテキスト

ストアを選ぶと OpenSpec ルートが変わりますが、そのストアを使うすべてのコード
リポジトリを検出・読み取りするわけではありません。ストアの instructions には、
ストア内の成果物と設定済みのコンテキストが見えます。コンポーネントのコードが
見えるのは、そのフォルダーをエージェントまたはエディターから利用でき、
エージェントが実際に読み取った場合だけです。

ワークセットを使うと、計画ストアと 2 つのコードリポジトリをまとめて開けます。
...
これにより、1 つの IDE ワークスペースで各フォルダーを確認できます。ただし、
ソースコンテキストをストアにコピーしたり、影響を受けるリポジトリを選択したり、
エージェントに編集権限を与えたりするものではありません。コンポーネントをまたぐ
永続的な事実は共有仕様に書き、たまたま確認したソースを計画者が覚えていることに
依存しないでください。

### 各リポジトリで実装を始める方法

明示的な `--store` も、より近い `openspec/` ルートもない場合、
`store: team-plans` ポインターによってコマンドはそのストアに送られます。
ただし、1 つのストアのタスクリストを、`apply` を実行したディレクトリごとに
分割するわけではありません。現在の OpenSpec はタスクをリポジトリへ振り分けません。

各コンポーネントに独立した適用・レビューサイクルが必要な場合は、中央ストアを
ポインターで指定する代わりに、ローカルの OpenSpec ルートを持たせて参照します。

```yaml
# checkout-api/openspec/config.yaml（checkout-web も同様）
schema: spec-driven
references:
  - team-plans
```

共有契約が承認され、ストアのメイン仕様として利用できるようになったら、
コンポーネント側の担当分について小さなローカル変更を作成します。
...
各リポジトリの instructions にある参照インデックスには、ストア仕様の概要と、
正確な取得コマンド `openspec show ... --store team-plans` が示されます。各ローカル
提案は共有契約を参照し、タスクにはそのコンポーネント内の作業だけを記述します。
続いて各リポジトリで個別に `/opsx:apply` を実行します。ルート解決により成果物と
実装編集のスコープはそのリポジトリに保たれます。サービスとフロントエンドの変更は、
それぞれ独立してテスト、レビュー、マージ、アーカイブできます。

共有ストアの変更がアクティブな間に実装を始める必要がある場合は、
`openspec show add-checkout-promo --store team-plans` で明示的に取得します。
参照インデックスに載るのは正規のストア仕様であり、アクティブなストア変更では
ありません。各実装がどの版の契約に従うかをレビュー担当者が確認できるよう、
ストアのブランチとコンポーネントのブランチをプルリクエストの説明で関連付けます。
...
4. defaultStore          グローバル設定でマシンの     → そのストア
                         デフォルトを指定
5. none of the above     このマシンにストア登録あり?  → 選択ヒント付きエラー
                         ストア登録なし?             → 現在のディレクトリ
                                                         (従来の挙動)
```
...
- **ベータ版の仕様です。** このページの内容は、名前、フラグ、ファイル形式、
  JSON キーを含め、リリースごとに変わる可能性があります。
- **1 台のマシンでは、ストア ID ごとにチェックアウトは 1 つです。** 同じ ID で
  2 つ目のチェックアウトを登録しようとすると失敗し、先に `store unregister`
  するためのヒントが表示されます。
- **同期は意図的に行いません。** OpenSpec は clone、pull、push しません。古い
  チェックアウトでは、自分で pull するまで古い仕様が見えます。参照はディスク上に
  ある内容からその場でインデックスされます。
- **空の計画フォルダーがまだ存在しない場合があります。** 新しいストアには、Git 上で
  `openspec/changes/`、`openspec/specs/`、`openspec/changes/archive/` がまだ存在
  しないことがあります。ベータ期間中は許容され、通常のコマンドがファイルを作ると
  これらのフォルダーも作成されます。
- **ポインターリポジトリはポインターのままです。** `openspec/config.yaml` だけを持ち、
  `store: <id>` を宣言しているリポジトリは、登録対象のストアチェックアウトではなく、
  外部化された計画を指すものとして扱われます。そのリポジトリを意図的にローカルの
  ストアルートへ変換したい場合は、先に `store:` 行を削除してください。
- **一部のコマンドは対象外です。** `templates` と非推奨の名詞形式
  （`openspec change show` など）は、現在のディレクトリだけを対象にします。`--store`
  はありません。`schemas` は正規のルート選択優先順位に従い、`--store <id>` を受け付け
  ますが、成功時の JSON 配列形式は変更しません。
- **マシンごとの状態はマシンごとです。** ストアレジストリとワークセットはローカル
  設定です。あなたのマシン上のレイアウトが共有計画にコミットされることはありません。
- **ワークセットには 2 つの起動スタイルがあります。** ワークスペースファイル、または
  フォルダーごとの追加フラグで起動できないツールは、opener として追加できません。
- **エージェント JSON には既知の大文字小文字の違いがあります。** ストア系のキーは
  snake_case、ワークフロー系は camelCase です。[エージェント契約](../agent-contract.md)
  に記載しています。統一はバージョン付きリリースまで延期されています。
