...
export function resolveSchema(name: string, projectRoot?: string): SchemaYaml {
  ...
  if (!schemaDir) {
    ...
    throw new Error(
      `スキーマ '${normalizedName}' が見つかりません。利用可能なスキーマ: ${availableSchemas.join(', ')}`
    );
  }
  ...
  try {
    ...
    throw new SchemaLoadError(
      `スキーマ '${schemaPath}' の読み取りに失敗しました: ${ioError.message}`,
      schemaPath,
      ioError
    );
  }
  ...
  try {
    ...
    if (err instanceof SchemaValidationError) {
      throw new SchemaLoadError(
        `スキーマ '${schemaPath}' が不正です: ${err.message}`,
        schemaPath,
        err
      );
    }
    ...
    throw new SchemaLoadError(
      `スキーマ '${schemaPath}' の解析に失敗しました: ${parseError.message}`,
      schemaPath,
      parseError
    );
  }
