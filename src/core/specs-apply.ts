...
function resolveTrustedSpecPath(specsRoot: string, specPath: string): {
  ...
  if (!isLexicallyWithin(specsRoot, specPath)) {
    throw new Error(`パスが許可されたディレクトリ外にあります: ${specPath}`);
  }
...
export async function retireSpec(
  ...
  if (options.deferDelete && options.verifyDisplaced === undefined) {
    throw new Error('遅延削除を行うには、退避したファイルの検証が必要です。');
  }
  ...
  try {
    ...
    throw new Error(
      `機能 '${update.id}' を廃止できません: 削除前に ${update.target} を検証できませんでした` +
  ...
  if (realSource !== undefined) {
    ...
    try {
      ...
      throw new Error(
        `機能 '${update.id}' を廃止できません: ${update.target} が ${mainSpecsDir} 内にあることを検証できませんでした` +
          `（${error instanceof Error ? error.message : String(error)}）。`
      );
    }
    if (!inside) {
      throw new Error(
        `機能 '${update.id}' を廃止できません: ${update.target} は ${mainSpecsDir} の外部へ解決されます。` +
          `外部ファイルを手動で削除するか、シンボリックリンクを置き換えてから再実行してください。`
      );
    }
  }
  ...
  try {
    ...
    throw new Error(
      `機能 '${update.id}' を廃止できません: ${update.target} の削除に失敗しました` +
        `（${(error as Error).message}）。手動で削除してからアーカイブを再実行してください。`
    );
  }
  ...
  if (!options.silent) {
    console.log(`${nominal} を廃止します: すべての要件が削除されました。`);
  }
...
export async function writeUpdatedSpec(
  ...
  const specName = update.id;
  console.log(`${options.displayPath ?? `openspec/specs/${specName}/spec.md`} に変更を適用しています:`);
