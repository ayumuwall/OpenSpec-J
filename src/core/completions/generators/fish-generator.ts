...
${helperFunctions}
${dynamicHelpers}
complete -c openspec -l no-color -f -d '色付き出力を無効化'
${topLevelCommands}
...
${commandCompletions}`;
  ...
  private generatePositionalCompletion(positionalType: string | undefined, condition: string): string[] {
    ...
    switch (positionalType) {
      ...
      case 'shell':
        ...
        break;
      case 'path':
        // -F はファイルシステム補完を再び有効にする。同じコンテキストの並列規則に
        // -f があり、Fish は --force-files なしではファイル補完を復元しない。
  ...
  /**
   * 次のトークンを値として受け取るフラグの長い名前と短い名前を収集する。
   */
  ...
  /**
   * コマンドの各単語が実際の位置に一致する Fish 条件を作成する。
   */
