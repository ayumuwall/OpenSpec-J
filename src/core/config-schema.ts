...
export function validateConfigKeyPath(path: string): { valid: boolean; reason?: string } {
  ...
  if (rawKeys.length === 0 || rawKeys.some((key) => key.trim() === '')) {
    return { valid: false, reason: 'キーのパスは空にできません' };
  }

  const rootKey = rawKeys[0];
  if (!KNOWN_TOP_LEVEL_KEYS.has(rootKey)) {
    return { valid: false, reason: `トップレベルキー "${rootKey}" は不明です` };
  }

  if (rootKey === 'featureFlags') {
    if (rawKeys.length > 2) {
      return { valid: false, reason: 'featureFlags の値は boolean のため、ネストされたキーは使えません' };
    }
    return { valid: true };
  }

  if (rawKeys.length > 1) {
    return { valid: false, reason: `"${rootKey}" はネストされたキーをサポートしません` };
  }
...
export function validateConfig(config: unknown): { success: boolean; error?: string } {
  try {
    ...
    return { success: false, error: '不明な検証エラー' };
  }
}
