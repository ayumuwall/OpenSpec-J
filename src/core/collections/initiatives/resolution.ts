...
export async function resolveSelectedInitiativeViewReference(
  ...
  if (!state) {
    throw new InitiativeResolutionError(
      `Initiative '${initiativeId}' は context store '${selected.id}' に見つかりませんでした。`,
...
export async function listInitiativeViewReferences(
  ...
  if (unreadable.length > 0) {
    status.push(makeDiagnostic(
      ...
      '一部の登録済み context store を読み取れませんでした。',
      {
        target: 'context_store',
        fix: 'openspec context-store doctor',
      }
  ...
  if (invalidCollections.length > 0) {
    status.push(makeDiagnostic(
      ...
      '一部の登録済み context store に不正な initiative が含まれています。',
      {
        target: 'initiative',
        fix: '不正な initiative フォルダ状態を修正してから再試行してください。',
      }
...
export async function resolveInitiativeViewReference(
  ...
  if (matches.length > 1) {
    throw new InitiativeResolutionError(
      `Initiative '${parsed.initiativeId}' は複数の context store に存在します。`,
