...
export function getBulkArchiveChangeSkillTemplate(): SkillTemplate {
  return {
    ...
    description: '複数の完了済み OpenSpec の変更を一括でアーカイブします。並行して進めた複数の変更をまとめてアーカイブするときに使用します。「openspec bulk-archive」「opsx bulk-archive」「openspec archive all」「openspec archive these changes」など、複数のアーカイブを求められた場合にも使用します。',
...
3. **一括検証 - 選択したすべての変更の状態を取得**
   ...
   タスク進捗を取得するため、同じ選択済みルートのフラグで \`openspec list --json\` を
   1 回実行します。取得に失敗する、JSON が不正、選択した変更が欠落・重複する、
   件数が不正な場合は、問題を報告し、一括処理の同期やアーカイブの前に停止します。
   ...
   b. **タスク完了状況** - list の応答の \`changes\` から、この変更と \`name\` が完全一致する項目を探す
      - \`totalTasks\` と \`completedTasks\` が非負の整数で、\`completedTasks <= totalTasks\` であることを必須とする
      - 未完了タスク数は \`totalTasks - completedTasks\`
      - CLI がスキーマの追跡対象タスクファイルを解決するため、カスタムのアーティファクト名、出力パス、glob にも対応する
      - アーティファクトの状態、\`tasks\` というアーティファクト ID、最上位に \`tasks.md\` がないことから完了を推測しない
      - CLI は \`x\`/\`X\` のチェックボックスマーカーだけを完了と数え、それ以外は未完了とする
      - \`totalTasks\` が 0 の場合は「タスクなし」と記録する
   ...
   d. **アーカイブ先** - 各変更の対象名を 1 回だけ算出し、その変更の \`<target-name>\` として記録する
      - 変更名がすでに \`YYYY-MM-DD-\` で始まる場合はそのまま使い、それ以外は現在の日付を \`YYYY-MM-DD-<name>\` として付ける（\`openspec archive\` と同じ規則）
      - \`<planningHome.changesDir>/archive/<target-name>\` が既に存在するか確認する
      - 存在する場合や、選択した別の変更が同じ対象名に解決される場合は、該当するすべての変更を \`Blocked\` とし、\`アーカイブディレクトリが既に存在します\` と記録する
      - ブロックされた変更は同期も移動もしない。手順 6 の表に \`Blocked\` と表示し、競合解決から除外する（その競合は他の変更だけで解決する）。手順 8d で失敗として記録する
      - 本仕様への書き込み前に確認することで、\`openspec archive\` と挙動を揃える。同期後に衝突が判明すると、アーカイブされない変更のために本仕様だけが書き換わってしまう
...
7. **一括操作の確認**
   ...
   - 準備完了のみの選択肢 — 手順6の表で \`Ready\` または \`Ready*\` と示した変更だけを処理し、
     残りは手順8dでスキップとして記録する。ただし \`Blocked\` の変更は \`アーカイブディレクトリが既に存在します\` の失敗のままとする。\`Ready*\` の変更と競合する相手がスキップされた場合は、
...
8. **確認済みの各変更をarchiveする**
   ...
   a. **対象に含めた仕様差分を同期**:
      - \`includedDeltas\` に項目がある変更だけで ${optionalWorkflow('sync', '`openspec-sync-specs` ワークフローをインラインで実行します（エージェントによる知的マージ）', '仕様差分から本仕様へのマージを自分でインライン実行します（エージェントによる知的マージ）')}。対象に含めたデルタパスだけを渡し、
   ...
   c. **archiveを実行**:
      ...
      対象名: 手順 3d でこの変更に記録した \`<target-name>\` をそのまま使います。ここで再計算しないでください。日付をまたぐ一括処理では、確認時と移動時の日付が変わってしまいます。

      **対象が既に存在するか確認:**
      - 手順 3 で確認済みでも、移動直前に再確認する。一括処理の途中で対象が作成される可能性がある
      - 存在する場合: \`アーカイブディレクトリが既に存在します\` と失敗を記録し、\`changeRoot\` はそのまま残す。手順 8a で既に同期した本仕様があれば報告し、残りの変更を続ける
      - 存在しない場合: \`changeRoot\` をアーカイブディレクトリへ移動する
      ...
      **入れ子への移動になっていないか確認:** 確認後に対象が作成された場合、\`mv\` は変更をその中へ移動して終了コード 0 を返します。\`<planningHome.changesDir>/archive/<target-name>/<change-directory-name>\`（末尾は \`changeRoot\` の最後のパス要素）が存在する場合、そのディレクトリを \`changeRoot\` へ戻し、\`アーカイブディレクトリが既に存在します\` と失敗を記録します。アーカイブ済みとは報告しません。
...
進行中の変更はありません。新しい変更を作成して始めてください。
\`\`\`
...
- アーカイブの対象名は手順 3d で当日の日付から 1 回だけ算出し、移動時にも再利用する: YYYY-MM-DD-<name>。すでに \`YYYY-MM-DD-\` 接頭辞がある名前はそのまま使用し、日付を二重に付けない
- アーカイブ先が存在する場合は、その変更だけ失敗として他を継続する
- 最初の本仕様書き込み前に、手順 3 ですべてのアーカイブ先を確認する。対象が存在する変更は同期も移動もしない
- 同期を求められた場合、含めた仕様差分がある各変更について ${optionalWorkflow('sync', '`openspec-sync-specs` ワークフローをインラインで実行する（エージェントが実施）', '仕様差分から本仕様へのマージをインラインで実行する（エージェントが実施）')}
...
8. **確認済みの各変更をarchiveする**
   ...
   a. **対象に含めた仕様差分を同期**:
      - \`includedDeltas\` に項目がある変更だけで ${SYNC_INLINE_HANDOFF}。対象に含めたデルタパスだけを渡し、
        その変更の \`excludedDeltas\` を無視するよう明示して、完了を待ちます
      - 競合については解決済みの順序で適用する
      - その変更用に取得したspecsルールのスナップショットをインライン同期へ渡し、
        同期側は指示を再取得せず再利用する
...
- アーカイブ先が存在する場合は、その変更だけ失敗として他を継続する
- 最初の本仕様書き込み前に、手順 3 ですべてのアーカイブ先を確認する。対象が存在する変更は同期も移動もしない
- 同期を求められた場合、含めた仕様差分がある各変更について ${SYNC_GUARDRAIL}
- delta ごとの \`includedDeltas\` と \`excludedDeltas\` の判断を実行へ引き継ぎ、含めた delta だけを同期・検証する
- 除外した delta はすべて、アーカイブ自体のスキップとして扱わず \`同期をスキップ\` と報告する
- 仕様同期の実行中にアーカイブしない。同期をインラインで実行し、\`changeRoot\` を移動する前に \`<planningHome.root>/openspec/specs/<capability-path>/spec.md\` の本仕様を検証する
- 仕様を確認または移動する前に、選択したルートごとに archive 入力を1回取得する
