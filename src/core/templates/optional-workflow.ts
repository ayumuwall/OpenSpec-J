...
function assertConditionalsWellFormed(text: string): void {
  ...
  if (unrecognized) {
    throw new Error(
      `任意ワークフローの条件分岐が不正です。認識できないマーカー: '${withoutMarkers
        .slice(unrecognized.index, unrecognized.index + 40)
        .split('\n')[0]}'。マーカーは [[opsx:if-workflow <id>]]、[[opsx:else]]、[[opsx:end]] です。`
    );
  }
  ...
  for (let i = 0; i < kinds.length; i += 3) {
    if (kinds[i] !== 'if' || kinds[i + 1] !== 'else' || kinds[i + 2] !== 'end') {
      throw new Error(
        '任意ワークフローの条件分岐が不正です。マーカーの順序が違うか、' +
          'ブロックが不完全です。各ブロックには [[opsx:if-workflow <id>]] ' +
          '... [[opsx:else]] ... [[opsx:end]] の完全な形式が必要で、入れ子にはできません。'
      );
    }
  }
}
...
export function assertWorkflowConditionalsResolved(text: string, reason: string): void {
  ...
  if (residual) {
    throw new Error(
      `${reason}: '${residual[0]}' が未解決です。任意ワークフローのブロックは ` +
        'getSkillTemplates()/getCommandTemplates() がインストール対象の' +
        'ワークフローに基づいて解決します。各ブロックには [[opsx:if-workflow <id>]] ... ' +
        '[[opsx:else]] ... [[opsx:end]] の完全な形式が必要です。'
    );
  }
}
