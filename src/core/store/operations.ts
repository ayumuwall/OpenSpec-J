...
function assertNoRegisteredStoreInside(
  ...
  const unregister = nested.map((other) => `openspec store unregister ${other.id}`).join('、次に ');
  throw new StoreError(
    `${storeRoot} 内に別の登録済みストアがあるため、削除しません: ${listed}。`,
    'store_remove_contains_registered_store',
    {
      target: 'store.root',
      fix: `先に内部のストアを登録解除または削除するか（${unregister}）、"openspec store unregister ${id}" を実行してファイルを削除せずに '${id}' の登録を解除してください。`,
    }
  );
}
