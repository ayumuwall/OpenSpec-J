...
このディレクトリは OpenSpec workspace です。context store、initiative、リポジトリ、フォルダをまたぐローカル作業ビューとして使います。
...
この workspace は initiative に紐付いていません。それでも、選択したリポジトリまたはフォルダに対する通常のローカルビューとして扱えます。
...
${formatGuidancePathList(linkedRoots)}`;
  ...
  const contextLines = resolvedContext
    ? [
        ...
        '- より広いコンテキストが context store 内に存在する場合がありますが、この workspace はデフォルトで選択した initiative を開きます。',
      ].join('\n')
    : [
        `- Context store: ${storedContextStore}`,
        `- Initiative: ${storedInitiativeId}`,
        '- このビューの解決済みローカルパスを更新するには `openspec workspace open --json` を実行してください。',
      ].join('\n');
...
- initiative と context-store ファイルは共有の調整コンテキストとして扱います。
- リンク済みリポジトリとフォルダは、ユーザーが選択したときのローカル実装コンテキストとして扱います。
- この OpenSpec バージョンでは、これらの境界は助言的なものです。編集時は判断とリポジトリの所有範囲を尊重してください。
