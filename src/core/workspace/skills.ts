...
export function parseWorkspaceSkillToolsValue(rawTools: string): string[] {
  ...
  if (raw.length === 0) {
    throw new Error(
      '--tools オプションには値が必要です。"all"、"none"、またはエージェント ID のカンマ区切りリストを指定してください。'
    );
  }
  ...
  if (tokens.length === 0) {
    throw new Error(
      '"all" または "none" を使わない場合、--tools オプションには少なくとも 1 つのエージェント ID が必要です。'
    );
  }
  ...
  if (normalizedTokens.some((token) => token === 'all' || token === 'none')) {
    throw new Error('"all" または "none" は、特定のエージェント ID と組み合わせられません。');
  }
  ...
  if (invalidTokens.length > 0) {
    throw new Error(`不正なエージェントです: ${invalidTokens.join(', ')}。利用可能な値: ${availableList}`);
  }
...
function getWorkspaceSkillTool(toolId: string): WorkspaceSkillCapableTool {
  ...
  if (!tool) {
    throw new Error(`不明な workspace skill agent '${toolId}' です。`);
  }
