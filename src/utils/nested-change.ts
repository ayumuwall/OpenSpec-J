 ...
 */
export const NESTED_CHANGE_ISSUE_MARKER = 'は変更ではなく、次の変更を内包するフォルダーです:';
...
export function describeNestedChange(finding: NestedChangeFinding): string {
  ...
  return (
    ...
    '変更は openspec/changes/ 直下のディレクトリである必要があります。' +
    '入れ子のディレクトリは OpenSpec に認識されず、それを内包するフォルダーが変更として表示されます。' +
    '入れ子のパスに対応しているのは openspec/specs/ 配下のみです。' +
    `各変更を直下へ配置できる名前（例: "${example}"）に変更してください。`
  );
}
