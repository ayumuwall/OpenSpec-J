...
export function validateSchemaName(
  ...
  if (!availableSchemas.includes(schemaName)) {
    throw new Error(
      `スキーマ '${schemaName}' が見つかりません。利用可能: ${availableSchemas.join(', ')}`
    );
  }
  return schemaName;
}
...
export function writeChangeMetadata(
  ...
  if (!parseResult.success) {
    throw new ChangeMetadataError(
      `メタデータが不正です: ${parseResult.error.message}`,
      metaPath
    );
  }
  ...
  try {
    ...
    throw new ChangeMetadataError(
      `メタデータの書き込みに失敗しました: ${ioError.message}`,
      metaPath,
      ioError
    );
  }
...
export function readChangeMetadata(
  ...
  try {
    ...
    const ioError = err instanceof Error ? err : new Error(String(err));
    throw new ChangeMetadataError(
      `メタデータの読み取りに失敗しました: ${ioError.message}`,
      metaPath,
      ioError
    );
  }
  ...
  try {
    ...
    throw new ChangeMetadataError(
      `メタデータファイルの YAML が不正です: ${parseError.message}`,
      metaPath,
      parseError
    );
  }
  ...
  if (!availableSchemas.includes(parseResult.data.schema)) {
    throw new ChangeMetadataError(
      `スキーマ '${parseResult.data.schema}' が見つかりません。利用可能: ${availableSchemas.join(', ')}`,
      metaPath
    );
  }
...
function readBooleanMarker(
  ...
  try {
    ...
    return unhonorable(`メタデータファイルを読み込めません (${message})`);
  }
  ...
  try {
    ...
    return mentioned ? unhonorable('ファイルが有効な YAML ではありません') : { declared: false };
  }
  ...
  if (result.success) {
    ...
    try {
      ...
      if (!listSchemas(projectRoot).includes(result.data.schema)) {
        return unhonorable(`schema: 不明なスキーマ '${result.data.schema}'`);
      }
