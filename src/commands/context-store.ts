...
async function prepareSetupInput(
  ...
  try {
    ...
    const shouldContinue = await confirm({
      message: `${asErrorMessage(error)}。それでもこの場所を使いますか？`,
      default: false,
    });

    if (!shouldContinue) {
      throw new ContextStoreError(
        'context store setup をキャンセルしました。',
        'context_store_setup_cancelled',
        {
          target: 'context_store.root',
          fix: '別のパスを選ぶか、後で setup を再実行してください。',
        }
      );
    }
...
async function confirmRemove(id: string, root: string, options: ContextStoreRemoveOptions): Promise<void> {
  ...
  if (options.json || !isInteractive()) {
    throw new ContextStoreError(
      '非対話で context-store ファイルを削除するには --yes を渡してください。',
  ...
  if (!confirmed) {
    throw new ContextStoreError(
      'context store remove をキャンセルしました。',
...
function printMutationHuman(title: string, payload: ContextStoreMutationOutput): void {
  ...
  console.log(`場所: ${formatPathForHuman(payload.context_store.root)}`);
  console.log('');
  console.log(`次: エージェントに ${payload.context_store.id} 内で initiative を作成するよう依頼してください。`);
}
...
function printCleanupHuman(title: string, payload: ContextStoreCleanupOutput): void {
  ...
  if (payload.files.deleted_path) {
    console.log(`削除: ${formatPathForHuman(payload.files.deleted_path)}`);
  } else if (payload.files.left_on_disk) {
    console.log(`ファイルは保持: ${formatPathForHuman(payload.files.left_on_disk)}`);
  } else if (!payload.files.deleted) {
    console.log(`ファイルは既に存在しません: ${formatPathForHuman(payload.context_store.root)}`);
  }

  for (const status of payload.status) {
    console.log(`${status.severity === 'warning' ? '注記' : '問題'}: ${status.message}`);
  }
}
