...
/** findings は full-v1 の items を部分化したものではなく、独立したレポートである。 */
...
export class ValidateCommand {
  async execute(itemName: string | undefined, options: ExecuteOptions = {}): Promise<void> {
    ...
    if (options.report !== undefined) {
      const message = options.report !== 'full' && options.report !== 'findings'
        ? `未知の検証レポート '${options.report}' です。`
        : itemName !== undefined
          ? '検証レポートと項目名は同時に指定できません。'
          : options.archived && bulk
            ? '検証レポートでアーカイブ済みとアクティブの対象範囲を同時に指定できません。'
            : !options.archived && !bulk
              ? '検証レポートには一括検証の対象範囲を明示する必要があります。'
              : undefined;
      if (message) {
        const fix = '項目名を指定せず、--report full|findings を --all、--changes、--specs、--archived のいずれかと組み合わせてください。アーカイブ済みとアクティブの対象範囲は同時に指定しないでください。';
        if (options.json) {
  ...
  private async runInteractiveSelector(root: ResolvedOpenSpecRoot, opts: { strict: boolean; json: boolean; concurrency?: string }): Promise<void> {
    ...
    const choice = await select({
      message: '何を検証しますか？',
      choices: [
        { name: 'すべて（変更 + 仕様）', value: 'all' },
        { name: '変更をすべて', value: 'changes' },
        { name: '仕様をすべて', value: 'specs' },
        { name: '変更または仕様を選択', value: 'one' },
      ],
    });
    ...
    if (items.length === 0) {
      console.error('検証できる項目が見つかりません。');
      process.exitCode = 1;
      return;
    }
    const picked = await select<{ type: ItemType; id: string }>({ message: '検証する項目を選択してください', choices: items });
    await this.validateByType(root, picked.type, picked.id, opts);
  }

  private printNonInteractiveHint(root: ResolvedOpenSpecRoot): void {
    console.error('検証対象がありません。次のいずれかを試してください:');
  ...
  private async validateDirectItem(root: ResolvedOpenSpecRoot, itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      if (isStoreSelectedRoot(root)) {
        ...
        console.error('--type change|spec を指定するか、openspec change validate / openspec spec validate を使用してください。');
      }
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(root: ResolvedOpenSpecRoot, type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    const nameProblem =
      type === 'change'
        ? folderStyleNameProblem(id, '変更名')
        : (id.split('/').map((segment) => folderStyleNameProblem(segment, '仕様 ID')).find(Boolean) ?? null);
    if (nameProblem) {
      if (opts.json) {
        console.log(
          JSON.stringify(
  ...
  private printReport(type: ItemType, id: string, report: { valid: boolean; issues: any[] }, durationMs: number, json: boolean, root: ResolvedOpenSpecRoot): void {
    ...
    if (report.valid) {
      console.log(`${type === 'change' ? '変更' : '仕様'} '${id}' は有効です`);
    } else {
      console.error(`${type === 'change' ? '変更' : '仕様'} '${id}' に問題があります`);
    }
  ...
  private printNextSteps(type: ItemType, id: string, root: ResolvedOpenSpecRoot, issues: Array<{ message: string }> = []): void {
    ...
    if (type === 'change' && nestedIssue) {
      bullets.push('- 入れ子の各変更を openspec/changes/ の直下へ移動し、名前空間を変更名に組み込んでください');
      bullets.push('- ドメインごとに入れ子にできるのは仕様のみです。変更ディレクトリは常に同じ階層に配置します');
      ...
      bullets.push('- 各 Requirement には少なくとも1つの #### Scenario: ブロックが必要');
      bullets.push('- --json で再実行すると構造化レポートを確認できます');
    }
  ...
  private printFindingsReport(full: BulkValidationResult, scope: BulkScope, json: boolean, root: ResolvedOpenSpecRoot): void {
    ...
    console.log(`対象範囲: ${scope}（${findings.report.totalItems}件）`);
    if (findings.itemFindings.length === 0) {
      console.log('検出事項のある項目はありません。');
    }
    ...
    console.log(`合計: ${totals.passed}件成功、${totals.failed}件失敗（全${totals.items}件）`);
  ...
  private async runBulkValidation(root: ResolvedOpenSpecRoot, scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean; findingsScope?: BulkScope }): Promise<void> {
    ...
    if (opts.findingsScope) {
      ...
      console.log(`結果: 成功 ${summary.totals.passed} / 失敗 ${summary.totals.failed} （計 ${summary.totals.items} 件）`);
  ...
  private async runArchivedTaskValidation(
    ...
    const spinner = !opts.json && !opts.noInteractive ? ora('アーカイブ済みの変更を検証中...').start() : undefined;
      ...
      try {
        ...
        if (incomplete > 0) {
          issues.push({
            level: 'ERROR',
            path: 'tasks.md',
            message: `${incomplete}件の未完了タスク（${progress.completed}/${progress.total}件完了）`,
          });
        }
      } catch (error: any) {
        issues.push({ level: 'ERROR', path: 'tasks.md', message: error?.message || '不明なエラー' });
      }
    ...
    console.log(`合計: ${summary.totals.passed}件成功、${summary.totals.failed}件失敗（全${summary.totals.items}件）`);
    process.exitCode = failed > 0 ? 1 : 0;
  }
}
