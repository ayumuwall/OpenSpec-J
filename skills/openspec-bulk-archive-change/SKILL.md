...
8. **確認済みの各変更をarchiveする**
   ...
   a. **対象に含めた仕様差分を同期**:
      - `includedDeltas` に項目がある変更だけで `openspec-sync-specs` ワークフローをインラインで実行します（エージェントによる知的マージ）。対象に含めたデルタパスだけを渡し、
...
- 同期を求められた場合、含めた仕様差分がある各変更について `openspec-sync-specs` ワークフローをインラインで実行する（エージェントが実施）
